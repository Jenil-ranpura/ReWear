...
| PATCH | `/users/me` | user | **editable profile** — name, optional phone (E.164-validated, '' clears), OPTIONAL password change gated on `currentPassword` re-auth (403 INVALID_CREDENTIALS when wrong); write-limited; shared `profileUpdateSchema`; field whitelist (`PROFILE_EDITABLE_FIELDS`) so role/points/email can never ride in; unknown keys stripped |
| GET | `/admin/items/pending` | admin | moderation queue (duplicate-flag badge via moderationReason) |
| PATCH | `/admin/items/:id/moderate` | admin | `{ action: APPROVE\|REJECT, reason? }` — PENDING only; conditional update (race-safe 409) + audit row |
...
- Admin panel: moderation queue (approve/reject with reason), users management (search/filter/ban), reports console, lightweight oversight
...
- **Editable profile** — reached from the CLICKABLE NAVBAR PROFILE (avatar/initial + name → `/dashboard/profile`) and a dashboard "Edit profile" link; edits name, phone (E.164, '' clears), and password (currentPassword re-auth required); validated by the shared `profileUpdateSchema` on BOTH sides; AuthContext gained `updateUser` so the navbar reflects edits instantly
...
1. **Admin REMOVE for live items** — `moderateItem` currently only transitions PENDING items; an APPROVED item cannot be taken down by an admin (owners can soft-delete their own). Next step: add a `REMOVE` moderation action (APPROVED → REMOVED + reason + audit row) to fully close the "remove inappropriate or spam items" requirement.
2. **P9 — Deployment:** prod Atlas, backend deploy, frontend deploy, smoke test.
3. **P10 — Demo seed expansion + demo-script rehearsal.**
4. **CI workflow** (`.github/workflows/ci.yml` — lint + tests + build, no service containers) is specced but not yet committed.
5. Parked ideas (not started, do not build unprompted): notification center, admin CSV export, ratings/reputation, "items like this" recommendations.
