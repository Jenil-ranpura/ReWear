...
- New users start with a **25-point signup grant, LEDGER-BACKED** (user decision, final): `registerUser` writes the user doc AND an `EARNED` (+25) `pointstransactions` doc in ONE transaction (`withTransactionRetry`), so the cache=Σledger invariant holds from the first millisecond. The number lives in `@rewear/shared-schemas` `SIGNUP_POINTS_GRANT` (the User model default mirrors it). Pre-fix accounts (cache 25, empty ledger) are reconciled by `npm run backfill:signup-grant` in apps/api — dry-run by default, `--write` to apply, idempotent, prints a full-DB reconciliation report. There is no top-up endpoint (no payments by design) — the seed funds the demo user.
